...
`prepare` はデフォルトで changed slides を対象にし，project refresh，table of contents 更新，bibliography 更新，build を実行します，stale output pruning は実行しません，対象と実行予定の step は次で確認できます，
...
stale output pruning は破壊的な操作なので，明示的に実行します，`--apply` を渡さない場合は dry-run として動作します，
