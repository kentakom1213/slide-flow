...
`prepare` はデフォルトで changed slides を対象にし，project refresh，table of contents 更新，bibliography 更新，build，stale output pruning を実行します，対象と実行予定の step は次で確認できます，
...
stale output pruning は明示的に実行することもできます，`--apply` を渡さない場合は dry-run として動作します，
